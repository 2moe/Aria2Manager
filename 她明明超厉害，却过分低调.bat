...
::
::YAwzoRdxOk+EWAjk
::fBw5plQjdCyDJGyX8VAjFBZVXxCHLleeA6YX/Ofr08uVq0xPee06VY7V3bGcbeQW/EDoeoYg03ROpMoeBRoKQiCOTysZjFJypGuCJcaRjy7zQ0mn4Xc2GnN9hGzezGNoKZ1hmcxj
...
::cRYluBh/LU+EWAnk
::YxY4rhs+aU+IeA==
::cxY6rQJ7JhzQF1fEqQJhe04EHmQ=
::ZQ05rAF9IBncCkqN+0xwdVtCHUrRaSXqVNU=
...
::dhA7uBVwLU+EWE2J+0d+JR40
::YQ03rBFzNR3SWATElA==
::dhAmsQZ3MwfNWATElA==
::ZQ0/vhVqMQ3MEVWAtB9wSA==
::Zg8zqx1/OA3MEVWAtB9wPBZbSUqPOQs=
::dhA7pRFwIByZRRmM4FYgO0EbAwOLKGOvBPsf5+W0veeIpwI8Rus+KqLa1LWJJfJc+EDocJc+02k6
::Zh4grVQjdCyDJGyX8VAjFBZVXxCHLleeA6YX/Ofr08uVq0xPee06VY7V3bGcbeQW/EDoeoYg03ROpMoeBRoKQpdut4nMR+i6WOxT0kJyWoovmcdVCs7dz+2rYOY4aq/mjlu4eY0GyyXe
::YB416Ek+Zm8=
::
::
...
echo;      ║ [7] 登录时自动更新bt-trackers(需admin) ║       MiWM@MXZMS2aSMM  M8,.:ZM    iSZMM         .ZMB       
echo;      ║ [8] 移除bt-trackers自动更新任务        ║       rMM@ZMZaMBXa2aMa  SBB07 iBZ@MaX8M,      XMM8         
echo;      ║ [9] 手动更新bt-trackers                ║         BMMM@MMM22aSZMr        MWi    M:   r@MWr           
echo;      ║════════════════════════════════════════║        ZM2ZMMMMMBS228MMS       M      .;aW80r              
echo;      ║ d=====(￣▽￣*) Secret/Password        ║       ,MW2MMMMMBMSaS0MMMa      .;:7aB8MMZM,                
echo;      ║ [10] Enable secret  启用密码           ║        ;MMZ0MWB@Ma22X@MMMM8MM@MMWM2MM0Z  @MW,              
echo;      ║ [11] Disable secret  禁用密码          ║          7  BZ  MMM@ZZM8M0 XM  Ma0MM@i    MXM0             
...
echo;      ║ [12] 访问发布原帖                      ║                          2MB7Za0BMBB2S2MBM2aM7              
echo;　　　║ [13] View IP address           　　　  ║                      :rXa@MMM8XMW8       0@7M               
echo;　　　║                                        ║                 XBMMMMMMMMBXZM7M          MMM               
echo;　　　║════════════════════════════════════════║               ,M8@MMBX:      MMM          :MM               
echo;　　　                                                                       ,MaZ;          XMa                         
...
if /i "%Memories%"=="7" Goto login
if /i "%Memories%"=="8" Goto remove
if /i "%Memories%"=="9" Goto update
if /i "%Memories%"=="10" Goto enable
if /i "%Memories%"=="11" Goto disable
if /i "%Memories%"=="12" Goto github
if /i "%Memories%"=="13" Goto ip
if /i "%Memories%"=="14" Goto tmoe.me
...
echo.若浏览器连接失败，建议您换用firefox或chromium内核的浏览器。
echo.若仍无法解决，建议您还原配置文件。
firewall.cpl 
...
start "" ".\index.html"     
ECHO.&ECHO. ※ 按任意键退出。※ &PAUSE >NUL 2>NUL& goto menu
...
::start "" ".\downloads"
::cd /d %~dp0
cd .\README\Program\OpenSSL
echo.即将安装vc++2019运行库，若已安装，则无需等待12秒，按任意键跳过。
start .\VC_redist.exe 
...
ECHO.&ECHO aria2※ 安装成功，若验证失败，请检查密码是否输入正确。按任意键返回。※ &PAUSE >NUL 2>NUL&goto menu
...
cd /d %~dp0
cd .\README\Program\OpenSSL
echo.即将安装vc++2019运行库，如已安装，则无需等待6秒，按任意键跳过。
start .\VC_redist.exe 
echo.6秒后，将调用openssl的随机函数生成一个24位字符的密码。
echo.If you already have vc2019++ installed, press any key to skip.
timeout /t 6
...
echo.密码文本保存在当前目录下的password.txt
notepad .\password.txt
ECHO.&ECHO ※ 密码启用成功 ※ &PAUSE >NUL 2>NUL&goto menu
...
cd /d %~dp0
Taskkill /F /IM aria2c.exe
start .\README\Program\bt-trackers-update\aria2-trackers-update.exe
echo;                    :BQBQMji          jB.                                                           
...
cd /d %~dp0
schtasks /create /f /tn "Aria2-BT-trackers-update" /tr "%cd%\README\Program\bt-trackers-update\aria2-trackers-update.exe" /sc ONLOGON
%windir%\system32\taskschd.msc /s
echo;                                                                                                    
echo;                                                                                                    
echo;                                           :;;XXXXSSSXXXX;;,                                        
