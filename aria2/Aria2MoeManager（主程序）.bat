...
echo;　　　║ [0] 一键安装 One-key installation      ║         MMMMM8 M@  rWMX  SWM@0ZMM2X@MWaS2X78MMMZ ,MX       
echo;　　　║ [1] 查看进程状态  View status          ║        ZMir;iS2X :MW: .BMW822a2SBM8rWMMMBMMMMMZMMMS        
echo;　　　║ [2] 重启服务 Restart service           ║        M;     ::0M  :MMM@S2ZaZ2a7WMM@0 Z0  8MMBW0          
echo;　　　║ [3] 停止服务 Stop service              ║       rM    B07BMWXWMBMM@M22Z0@MMMMi  raZX:  rM8:          
echo;　　　║ [4] 卸载  uninstall                    ║       2M   BMZB:WBMM72MM:XM@MBZX:   .MX, SMM.  :BMa        
echo;　　　║ [5]  FAQ                               ║       ZM a0WM2MrMWMM0MMZiiMX        SM   ;MMW     MM;      
echo;　　　║ [6] 退出本工具 Quit                    ║        MBX  B@MM80@MX:  MZ;X02       MMXi;8@       iM:     
echo;      ║════════════════════════════════════════║       M0   rBMWaZX8M   M,   MMZ       ,a0ai    ,:.  MS     
echo;      ║   Update 更新--o(*^▽^*)┛                ║      ;M  ;MMSMBX2a2Mi XM    2MW       .            MM      
echo;      ║                                        ║       MiWM@MXZMS2aSMM  M8,.:ZM    iSZMM         .ZMB       
echo;      ║ [7] 手动更新bt-trackers                ║       rMM@ZMZaMBXa2aMa  SBB07 iBZ@MaX8M,      XMM8         
...
if /i "%Memories%"=="0" Goto install 
if /i "%Memories%"=="1" Goto status
if /i "%Memories%"=="2" Goto restart
if /i "%Memories%"=="3" Goto stop
if /i "%Memories%"=="4" Goto uninstall
if /i "%Memories%"=="5" Goto faq 
if /i "%Memories%"=="6" Goto exit
if /i "%Memories%"=="70" Goto update-manager
if /i "%Memories%"=="7" Goto update-tracker
if /i "%Memories%"=="8" Goto remove
if /i "%Memories%"=="9" Goto enable
if /i "%Memories%"=="10" Goto disable
if /i "%Memories%"=="11" Goto github
if /i "%Memories%"=="12" Goto ip
if /i "%Memories%"=="13" Goto plugin
if /i "%Memories%"=="14" Goto tmp
if /i "%Memories%"=="15" Goto tmoe.me
::if /i "%Memories%"=="15" Goto auto
...
if /i "%Memories%"=="egg" Goto 彩蛋
if /i "%Memories%"=="Easter egg" Goto 彩蛋
::if /i "%Memories%"=="17" Goto tmoe.me
if /i "%Memories%"=="test" Goto test
if /i "%Memories%"=="plugin" Goto plugin
if /i "%Memories%"=="a" Goto quick-start-annie
if /i "%Memories%"=="aria2" Goto aria2c
...
echo.示例2：输入 aria2 "https://baidu.com"
echo.示例3：输入 a https://baidu.com
echo.您即将进入兼容模式,将打开一个新窗口，请在该窗口下进行操作。
...
echo.[9] 删除所有插件
echo.
set /p plugins=请输入选项数字并按回车键。Please enter the option number and press Enter. オプション番号を入力してENTER キーを押してください：
...
:quick-start-annie
cd /d %~dp0
start .\data\Program\bat\annie.bat
goto menu
...
timeout /t 1
start .\data\Program\bat\annie-conf.bat
echo.
echo.annie是一款优秀的开源应用，你可以用它来解析主流网站的视频，并自动调用aria2下载。
echo.
::echo.启用该插件需要联网下载，请问您是否同意。（y/n）
echo.
echo.下载的任务列表可以在ariaNG网页进行管理。
echo.使用指南01：在打开的annie窗口内，输a url或aria2 url并按回车，直接下载（普通会员所能看到的）视频的最高画质版。（大会员视频需加载cookies，见指南2）
echo.示例1： 输入a https://www.bilibili.com/video/av10086206?p=1 再按回车即可下载。
echo.示例2： 输aria2 https://www.bilibili.com/video/av10086206?p=1
echo.若url包含特殊字符，则需用半角(英文)引号包围。示例3：输入a "https://www.bilibili.com/video/av10086206?p=1"
echo.指南2：下载会员视频或1080P+高画质视频需要加载cookies文件。
echo.使用浏览器插件(例如："edit this cookie"）将视频网站的cookies导出为netscape http格式,并命名为cookies.txt,最后将其覆盖于当前目录下的.\data\config\cookies.txt
echo.一个cookies.txt文件可以包含多个视频网站的会员cookie信息,千万请不要把该文件共享出去。
echo.
echo.指南3：输入ai url或a -i url获取所要下载视频的详细信息。
echo.之后你可以通过-f参数来指定画质。例如annie -f 80 url 或者是a -f 80 url 
echo.指南4：输入ap url或a -p url下载整个播放列表。

echo.隐藏传送门：在Aria2 Moe Manager主菜单下输a并按回车，能直接打开annie窗口。
::start .\data\Program\bat\annie.bat

ECHO.&ECHO. ※ Press any key to return. 按任意键返回。※ &PAUSE >NUL 2>NUL& goto plugin
:delete-annie
cd /d %~dp0
...
start "" ".\index.html"
TaskList /FI "IMAGENAME eq aria2c.exe" /FO LIST
ECHO.&ECHO. ※ 还原配置文件成功，按任意键返回。 ※ &PAUSE >NUL 2>NUL& goto menu
...
TaskList /FI "IMAGENAME eq aria2c.exe" /FO LIST 
ECHO.&ECHO. ※ 禁用成功，按任意键返回※ &PAUSE >NUL 2>NUL&goto menu
:install
Taskkill /F /IM aria2c.exe
echo.正在初始化。。。
::start "" ".\downloads"
...
::start .\aria2-trackers-update.vbs
::cd ..\bat
cd .\data\Program\bat
start .\success.bat
::cd ..\bin
...
pause > nul
rmdir /Q /S %current_dir% & echo 所有文件都已删除。若提示文件被占用，请关闭浏览器中已打开的ariaNG页面。
::echo.将自动打开任务计划程序，请手动检查计划任务是否有残留。
...
TaskList /FI "IMAGENAME eq aria2c.exe" /FO LIST  
ECHO.&ECHO. ※服务重启完成，若edge-html无法连接，则您可以尝试把地址粘贴到firefox或chromium内核的浏览器。按任意键返回！※ &PAUSE >NUL 2>NUL&goto menu
...
cd /d %~dp0
cd .\data\Program
echo.Are you sure to delete update-module?
pause
...
ECHO.&ECHO. ※移除完成，按任意键返回！※ &PAUSE >NUL 2>NUL&goto menu
...
@echo off
cd /d %~dp0
if exist ".\data\Program\mingw\bin\git.exe" (goto git-pull) else (goto unzip)
:unzip
echo.检测到更新模块未启用，正在为您启用。
.\data\Program\update-module.7z.exe -y
timeout /t 3
...
echo;                                         ii;i:        .;rr::rX;:  
.\data\Program\mingw\bin\git.exe reset --hard 
.\data\Program\mingw\bin\git.exe pull origin win64build
::查看更新日志。 
type .\data\Program\log\update.log    
ECHO.&ECHO. ※更新成功，按任意键返回！※ &PAUSE >NUL 2>NUL&goto menu
...
Taskkill /F /IM aria2c.exe
cd .\data\Program\bin
.\sed.exe -i "/max-concurrent-downloads=/c max-concurrent-downloads=10" ..\..\..\aria2.conf
.\sed.exe -i "/max-connection-per-server=/c max-connection-per-server=16" ..\..\..\aria2.conf
.\sed.exe -i "/min-split-size=/c min-split-size=1M" ..\..\..\aria2.conf
.\sed.exe -i "/split=/c split=16" ..\..\..\aria2.conf
cd ..\..\..\
.\aria2c.exe --conf-path=aria2.conf
echo;                                                                                                    
echo;                                                                                                    
...
:128
cd .\data\Program\bin
.\sed.exe -i "/max-concurrent-downloads=/c max-concurrent-downloads=7" ..\..\..\aria2.conf
.\sed.exe -i "/max-connection-per-server=/c max-connection-per-server=128" ..\..\..\aria2.conf
.\sed.exe -i "/min-split-size=/c min-split-size=512K" ..\..\..\aria2.conf
.\sed.exe -i "/split=/c split=128" ..\..\..\aria2.conf
cd ..\..\..\
echo;                                                                                                    
echo;                                                                                                    
echo;                                        ..,.,..                              iWMMMM@;               
...
:233
cd .\data\Program\bin
.\sed.exe -i "/max-concurrent-downloads=/c max-concurrent-downloads=4" ..\..\..\aria2.conf
.\sed.exe -i "/max-connection-per-server=/c max-connection-per-server=233" ..\..\..\aria2.conf
.\sed.exe -i "/min-split-size=/c min-split-size=256K" ..\..\..\aria2.conf
.\sed.exe -i "/split=/c split=128" ..\..\..\aria2.conf
cd ..\..\..\
echo;                                                                                                    
echo;                                                                                                    
...
cd .\data\Program\bin
.\sed.exe -i "/max-concurrent-downloads=/c max-concurrent-downloads=2" ..\..\..\aria2.conf
.\sed.exe -i "/max-connection-per-server=/c max-connection-per-server=499" ..\..\..\aria2.conf
.\sed.exe -i "/min-split-size=/c min-split-size=128K" ..\..\..\aria2.conf
.\sed.exe -i "/split=/c split=499" ..\..\..\aria2.conf
cd ..\..\..\
...
:999
cd .\data\Program\bin
.\sed.exe -i "/max-concurrent-downloads=/c max-concurrent-downloads=1" ..\..\..\aria2.conf
.\sed.exe -i "/max-connection-per-server=/c max-connection-per-server=999" ..\..\..\aria2.conf
.\sed.exe -i "/min-split-size=/c min-split-size=10K" ..\..\..\aria2.conf
.\sed.exe -i "/split=/c split=999" ..\..\..\aria2.conf
cd ..\..\..\
...
:github
start "" "https://github.com/2moe/Aria2Manager/releases"
ECHO.&ECHO. ※ヾ(≧▽≦*)o按任意键返回！※ &PAUSE >NUL 2>NUL&goto menu
...
::已禁用:64
Taskkill /F /IM aria2c.exe
copy /y "%~dp0\data\Program\32\aria2c.exe"  "%~dp0\aria2c.exe"
...
:test
start ""  "%~dp0\data\License\test.bat"  
